...
def start_message(message):
    bot.send_message(message.chat.id, text="Напиши навзание страны!")
...
def handle_country(message):
    ...
    if country_url:
        bot.send_message(message.chat.id, text="Окей, теперь название города")
...
def send_pic(message):
    ...
    if int(weather[1]) in range(-5,6):
        if 'дождь'in str.lower(weather[3]):
            ...
            bot.send_message(message.chat.id, text="На улице холодно и мокро, советую надеть теплую куртку с капюшоном и кофту.\n"
                                                    "Не забудь про шапку!")
        else:
    ...
    if int(weather[1]) in range(10,14):
        if 'дождь' in str.lower(weather[3]):
            ...
            bot.send_message(message.chat.id, text="На улице свежо и идет дождь, советую надеть легкую куртку и гольф.\n"
                                                    "Не забудь ЗОНТИК!")  
        else:
    ...
    if int(weather[1]) in range(14,17):
        if 'дождь' in str.lower(weather[3]):
            ...
            bot.send_message(message.chat.id, text="На улице тепло и идет дождь, надевай худи и прихвати что-нибудь от дождя.\n")
            
        else:
            bot.send_sticker(message.chat.id, random.choice(sun_14_16))
            bot.send_message(message.chat.id, text="На улице тепло, поэтому снимай куртку(если ты не мерзляк) и надевай худи\n")
            
    if int(weather[1]) in range(17,21):
        if 'дождь' in str.lower(weather[3]):
            bot.send_sticker(message.chat.id, random.choice(rain_17_20))
            bot.send_message(message.chat.id, text="На улице почти жара и идет дождь, майка и легкая джинсовка будет идеально.\n"
                                                   "Не забудь ЗОНТИК!")
        else:
            bot.send_sticker(message.chat.id, random.choice(sun_17_20))
            bot.send_message(message.chat.id, text="На улице почти жара, надевай футболку и прихвати легкую накдидку.\n")
